...
both_effect <- ggpredict(hypfour, terms = c("PupsTot","Status"), bias_correction = TRUE)
...
interaction <- ggplot(both_effect,aes(x=x,y=predicted, color = group)) + 
  ...
  geom_point(data = testingit, aes(x = PupsTot, y = PropInRange, color = Status), size = 6, shape = 16) + # Add raw data points
  labs(title = "", 
